...
function openVideo(id, no_history) {
  if (no_history !== true) updateModelFunction("openVideo", id);
...
function openProfile(id, no_history) {
  if (no_history !== true) updateModelFunction("openProfile", id);
...
function openGroupProfile(id, no_history) {
  if (no_history !== true) updateModelFunction("openGroupProfile", id);
...
function searchVideo() {
  ...
  if ($search_cat == undefined) $search_cat = "";
  if ($video_id == undefined) $video_id = "";

  $.get(
    "/home/ajax/video-search/?keyword=" +
      $search_text +
      "&category_id=" +
      $search_cat +
      "&video_id=" +
    ...
    function(data) {
      ...
      if (data.content == "") $("#video_search_res").html("No result!");
        ...
        if (key == "error") {
          $.each(val, function(key2, val2) {
            $("#login-form #messages").html(
              '<div class="alert alert-danger" role="alert">' + val2 + "</div>"
            );
          });
        } else {
          $("#login-form #messages").html(
...
function openVideoOnly() {
  ...
  $("#feature-info").html("loading...");
  var content =
    '<iframe style="width: 100%;height: 600px" frameborder="0" allowfullscreen src="' +
    video_id +
    '"></iframe>';
  $("#feature-info").html(content);
  $("#featureModal").modal("show");
...
function updateModelButtons() {
  if (model_links.length > model_links_current_pos) $(".model-foward").show();
  else $(".model-foward").hide();
