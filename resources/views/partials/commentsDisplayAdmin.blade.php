    ...
    <?php if($comment['parent_id'] == 0){ ?>
        <div id="forcomment_<?php echo uid($comment['id']) ?>" class="reply_comment row" style="clear: both;">
            <div style="width: 65%;float: left;margin-left: 25px;">
                <input class="comment_box" type="text" id="comment_text_<?php echo uid($comment['id']) ?>" placeholder="Text goes here..." style="width:100%;height: 33px;" />
            </div>
            <div style="float: left;">
                <a href="#" onclick="postComments('<?php echo ($fk_id) ?>', '<?php echo ($table) ?>', '<?php echo uid($comment['id']) ?>', 'comment_text_<?php echo uid($comment['id']) ?>')" class="btn btn-primary">Submit</a>
            </div>
        </div>
    <?php } ?>
